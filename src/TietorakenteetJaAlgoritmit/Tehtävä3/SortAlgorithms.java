package TietorakenteetJaAlgoritmit.Tehtävä3;
...
public class SortAlgorithms {
    ...
    public static void mergeSort() {
        ...
        System.out.println("\nMergeSort laskurin arvo oli " + laskuri);
        Lue.merkki();
    }
    ...
    public static void merge(int t[], int p, int q, int r) {
        ...
        int i = p, j = q + 1, k = 0;
        laskuri = laskuri + 2;                      // whilen kaksi vertailua // eka kierros
        while (i < q + 1 && j < r + 1) {
            laskuri++;                              // if:n vertailuehto
            if (t[i] < t[j]) {
                tau[k++] = t[i++];
            } else {
                tau[k++] = t[j++];
            }
            laskuri = laskuri + 2;                  // whilen kaikki muut kierrokset
        }
        //toinen osataulukko käsitelty, siirretään toisen käsittelemättömät
        laskuri++;                                  // 1. whilen eka kierros
        while (i < q + 1) {
            tau[k++] = t[i++];
            laskuri++;                              // 1. whilen muut kierrokset
        }
        laskuri++;                                  // 2. whilen eka kierros
        while (j < r + 1) {
            tau[k++] = t[j++];
            laskuri++;                              // 2. whilen muut kierrokset
        }
        //siirretään yhdiste alkuperäiseen taulukkoon
        laskuri++;                                  // for-loopin eka kierros
        for (i = 0; i < k; i++) {
            t[p + i] = tau[i];
            laskuri++;                              // for-loopin muut kierrokset
        }
    }
    ...
    public static void quickSortLoop() {
        ...
        System.out.println("Quicksort: Vertailujen lukumäärä: laskuri=" + laskuri);
    }
